...
def apply_rotary_emb(
    ...
    out = torch.cat((((2-lamb)**0.5)*t_left, ((lamb)**0.5)*t_transformed, t_right), dim=-1)
...
class RotaryEmbedding(Module):
    ...
    def rotate_queries_and_keys(self, q, k, seq_dim = None):
        ...
        rotated_q = apply_rotary_emb(freqs, q, scale = scale, seq_dim = seq_dim, start_index=hidden_dim-freqs.shape[-1])
        rotated_k = apply_rotary_emb(freqs, k, scale = scale ** -1, seq_dim = seq_dim, start_index=hidden_dim-freqs.shape[-1])
