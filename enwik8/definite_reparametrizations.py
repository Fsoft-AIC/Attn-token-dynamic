...
class AntiSym(nn.Module):
    def __init__(self, size: int):
        ...
        nn.init.kaiming_uniform_(self.T, a=math.sqrt(5))
...
class QKVParametrization(nn.Module):
    def __init__(self, size: int, wneg: float, aneg: float):
        ...
        with torch.no_grad():
            ...
            nn.init.kaiming_uniform_(qinit, a=math.sqrt(5))
...
class QParametrization(nn.Module):
    def __init__(self, size: int):
        super().__init__()
        self.q = nn.Parameter(torch.zeros(size, size))
        nn.init.orthogonal(self.q)  # node 2
    
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        return self.q


class KParametrization(nn.Module):
    def __init__(self, size: int, neg: float, q: QParametrization):
        super().__init__()
        self.size = size
        self.Wsym_generator = Definite(size, neg)
        self.q = q
        self.T_generator = AntiSym(size)

        # K init as kaiming
        kinit = torch.empty(size, size)
        nn.init.kaiming_uniform_(kinit, a=math.sqrt(5))
        f = q(None) @ kinit.T
        self.Wsym_generator.L_unconstrained.data = torch.cholesky(f@f.T+1e-3*torch.eye(size), upper=False)


    def forward(self, X: torch.Tensor) -> torch.Tensor:
        Wsym = self.Wsym_generator()
        Q = self.q(X)
        antisym = self.T_generator()
        return (torch.linalg.inv(Q + torch.eye(self.size, device=Q.device)*1e-5) @ (Wsym + antisym)).transpose(0, 1)

class VParametrization(nn.Module):
    def __init__(self, size: int, neg: float, q: QParametrization, k: KParametrization):
        super().__init__()
        self.size = size
        self.Asym_generator = Definite(size, neg)
        self.q = q
        self.k = k
        self.T_generator = AntiSym(size)

    def forward(self, X: torch.Tensor) -> torch.Tensor:
        Asym = self.Asym_generator()
        Q = self.q(X)
        K = self.k(X)
        antisym = self.T_generator()
        return torch.linalg.inv(Asym + antisym + torch.eye(self.size, device=Q.device)*1e-5) @ Q @ K.transpose(0, 1)
